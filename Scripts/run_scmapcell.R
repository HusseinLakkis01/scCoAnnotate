...
run_scmapcell <- function(RefPath,LabelsPath,TestPaths,OutputDirs){
  "
  ...
  i=1
    for(Test in TestPaths){
      ...
      end_time <- Sys.time()
      Testing_Time_scmapcell <- as.numeric(difftime(end_time,start_time,units = 'secs'))
      ...
      write.csv(Testing_Time_scmapcell,paste('scmapcell','_test_time.csv', sep = ''),row.names = FALSE)
      i = i+1
  }
}
...
# Run scmap-cell
run_scmapcell(ref,labs, test, output_dir)
# Output session info
sessionInfo()
