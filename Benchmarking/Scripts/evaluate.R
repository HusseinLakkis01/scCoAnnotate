...
evaluate <- function(TrueLabelsPath, PredLabelsPath, Indices = NULL){
  ...
  conf_F1 <- table(true_lab,pred_lab,exclude = c('unassigned','Unassigned','Unknown','rand','root','Node','ambiguous','unknown'))
  ...
  num_unlab <- sum(pred_lab == 'unassigned') + sum(pred_lab == 'Unassigned') + sum(pred_lab == 'rand') + sum(pred_lab == 'Unknown') + sum(pred_lab == 'unknown') + sum(pred_lab == 'Node') + sum(pred_lab == 'ambiguous') + sum(pred_lab == 'root')
